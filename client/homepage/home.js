...
async function getTopRatedMovies() {
    ...
    if (!token) {
        movieListBox.innerHTML = `
            <p id="movie-message">Please log in to see movies.</p>
        `;
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            movieListBox.innerHTML = `
                <p id="movie-message">${movies.error}</p>
            `;
            return;
        }
        ...
        movieListBox.innerHTML = `
            <p id="movie-message">Could not connect to server.</p>
        `;
    }
}
...
function renderMovies(movies) {
    ...
    if (movies.length === 0) {
        movieListBox.innerHTML = `
            <p id="movie-message">No movies found.</p>
        `;
        return;
    }
